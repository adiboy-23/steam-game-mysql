...
const DeveloperPage = () => {
    ...
    axios.put(`http://localhost:3001/api/games/${GameId}`, updateGameWithoutId)
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          // If the response indicates an error (e.g., game doesn't exist)
          console.error('Error updating the game:', response.data.error);
  
          // Alert for game not existing
          if (response.data.error && response.data.error === 'GameId does not exist') {
      ...
      .catch((error) => {
        ...
        // Handle backend errors that might not have been caught earlier
  ...
  return (
    <div className="container">
      <h1 className="header">Welcome Developer! {authenticatedDeveloperId}</h1>
